...
class NodeEdgeBlock(nn.Module):
    ...
    def forward(self, X, E, A, L, T, y, node_mask):
        ...
        A_mask = X_mask.unsqueeze(-1) # bs, n, 1, 1
        L_mask = X_mask.unsqueeze(-1) # bs, n, 1, 1
        T_mask = X_mask.unsqueeze(-1) # bs, n, 1, 1
        ...
        A1 = A1.reshape(A.size(0), A.size(1), 1, A.size(2), self.n_head, self.df)
        A2 = self.a_mul(A) * A_mask
        A2 = A2.reshape(A.size(0), 1, A.size(1), A.size(2), self.n_head, self.df)
        A3 = A1 * A2
        YA = A3 + A3 * Y.unsqueeze(3) + Y.unsqueeze(3)   # bs, n, n, n_head, df
        YA = YA.mean(2)        # bs, n, n_head, df

        # Incorporate dangle features to the self attention scores
        L1 = self.l_add(L) * L_mask # bs, n, dx
        L1 = L1.reshape(L.size(0), L.size(1), 1, L.size(2), self.n_head, self.df)
        L2 = self.l_mul(L) * L_mask
        L2 = L2.reshape(L.size(0), 1, L.size(1), L.size(2), self.n_head, self.df)
        L3 = L1 * L2
        YL = L3 + L3 * Y.unsqueeze(3) + Y.unsqueeze(3)   # bs, n, n, n_head, df
        YL = YL.mean(2)        # bs, n, n_head, df

        # Incorporate dangle features to the self attention scores
        T1 = self.t_add(T) * T_mask # bs, n, dx
        T1 = T1.reshape(T.size(0), T.size(1), 1, T.size(2), self.n_head, self.df)
        T2 = self.t_mul(T) * T_mask
        T2 = T2.reshape(T.size(0), 1, T.size(1), T.size(2), self.n_head, self.df)
        T3 = T1 * T2
        YT = T3 + T3 * Y.unsqueeze(3) + Y.unsqueeze(3)   # bs, n, n, n_head, df
        ...
        newA = YA.flatten(start_dim=3)            # bs, n, dx
        ya1 = self.y_a_add(y)[:, None, None, :]         # bs, 1, dx
        ya2 = self.y_a_mul(y)[:, None, None, :]
        newA = ya1 + ya2 * newA + newA            # bs, n, dx
        newA = self.a_out(newA) * A_mask          # bs, n, da

        # Incorporate y to L and output L
        newL = YL.flatten(start_dim=3)            # bs, n, dx
        yl1 = self.y_l_add(y)[:, None, None, :]         # bs, 1, dx
        yl2 = self.y_l_mul(y)[:, None, None, :]
        newL = yl1 + yl2 * newL + newL            # bs, n, dx
        newL = self.d_out(newL) * L_mask          # bs, n, dd

        # Incorporate y to T and output T
        newT = YT.flatten(start_dim=3)            # bs, n, dx
        yt1 = self.y_t_add(y)[:, None, None, :]         # bs, 1, dx
        yt2 = self.y_t_mul(y)[:, None, None, :]
...
class NNCombCat(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_in_A = nn.Sequential(nn.Embedding(self.input_dims['A'], hidden_mlp_dims['A'], 0),
                                      nn.Linear(hidden_mlp_dims['A'], hidden_mlp_dims['A'], 0), nn.ReLU(),
                                      nn.Linear(hidden_mlp_dims['A'], hidden_dims['da']), nn.ReLU())
        
        self.mlp_in_L = nn.Sequential(nn.Embedding(self.input_dims['L'], hidden_mlp_dims['L'], 0),
                                      nn.Linear(hidden_mlp_dims['L'], hidden_mlp_dims['L'], 0), nn.ReLU(),
                                      nn.Linear(hidden_mlp_dims['L'], hidden_dims['dl']), nn.ReLU())
        
        self.mlp_in_T = nn.Sequential(nn.Embedding(self.input_dims['T'], hidden_mlp_dims['T'], 0),
                                      nn.Linear(hidden_mlp_dims['T'], hidden_mlp_dims['T'], 0), nn.ReLU(),
    ...
    def forward(self, X, E, A, L, T, y, node_mask):
        ...
        A = torch.log_softmax(A, dim=-1) * A_mask.float()
        L = torch.log_softmax(L, dim=-1) * L_mask.float()
        T = torch.log_softmax(T, dim=-1) * T_mask.float()

...
def loss_fn(graph, con_diffusion, cat_diffusion, model):
    ...
    A, L, T = A.long().to(device), L.long().to(device), T.long().to(device)
    ...
    # Add noise to angles
    A_mask = get_triplet_mask(E_t.argmax(-1).long(), model.module.input_sizes['A'])
    L_mask = get_line_mask(E_t.argmax(-1).long(), model.module.input_sizes['L'])
    T_mask = get_twist_mask(E_t.argmax(-1).long(), model.module.input_sizes['T'])
    A, A_t = cat_diffusion.forward_process(A, t, A_mask, model.module.input_dims['A'])
    L, L_t = cat_diffusion.forward_process(L, t, L_mask, model.module.input_dims['L'])
    T, T_t = cat_diffusion.forward_process(T, t, T_mask, model.module.input_dims['T'])

    # Predict noise and calculate loss
    pred_X, pred_E, pred_A, pred_L, pred_T, _ = model(X_t.argmax(dim=-1).long(), 
                                                      E_t.argmax(dim=-1).long(), 
                                                      A_t.argmax(dim=-1).long(),
                                                      L_t.argmax(dim=-1).long(), 
                                                      T_t.argmax(dim=-1).long(), y_t, node_mask)
    ...
    A_loss = cat_diffusion.get_loss(A, A_t, pred_A, t, A_mask, model.module.input_dims['A'])
    L_loss = cat_diffusion.get_loss(L, L_t, pred_L, t, L_mask, model.module.input_dims['L'])
    T_loss = cat_diffusion.get_loss(T, T_t, pred_T, t, T_mask, model.module.input_dims['T'])
...
def val_fn(val_dataset, con_diffusion, cat_diffusion, model, decode_atom, decode_bond, epoch, config):
    ...
    scale = 32 * torch.cuda.device_count() if torch.cuda.is_available() else 32
    l = samples // scale + 1
    mols = []
    for i in range(1, l + 1):
        bs = min(scale, samples - (i - 1) * scale)
        new_mols = sample(model, con_diffusion, cat_diffusion, bs, model.module.__class__.__name__, val_dataset.dataset,
...
def evaluate(val_dataset, con_diffusion, cat_diffusion, model, config, decode_atom, decode_bond, epoch, device):
    ...
    for j in tqdm(range(i, -1, -1), position=1, leave=False):
        ...
        y_t = torch.hstack((y[:,:-1], t.unsqueeze(-1)))
        pred_X_0, pred_E_0, pred_A_0, pred_L_0, pred_T_0, _ = model(X_0.argmax(dim=-1).long(), 
                                                                    E_0.argmax(dim=-1).long(), 
                                                                    A_0.argmax(dim=-1).long(), 
                                                                    L_0.argmax(dim=-1).long(), 
                                                                    T_0.argmax(dim=-1).long(), y_t, node_mask)
        ...
        A_mask = get_triplet_mask(E_0.argmax(-1).long(), model.module.input_sizes['A'])
        L_mask = get_line_mask(E_0.argmax(-1).long(), model.module.input_sizes['L'])
        T_mask = get_twist_mask(E_0.argmax(-1).long(), model.module.input_sizes['T'])
        A_0 = cat_diffusion.reverse_process(A_0.argmax(dim=-1).long(), pred_A_0, t, A_mask, model.module.input_dims['A'])
        L_0 = cat_diffusion.reverse_process(L_0.argmax(dim=-1).long(), pred_L_0, t, L_mask, model.module.input_dims['L'])
        T_0 = cat_diffusion.reverse_process(T_0.argmax(dim=-1).long(), pred_T_0, t, T_mask, model.module.input_dims['T'])

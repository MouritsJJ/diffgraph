...
def sample_batch(n_samples, dataset, model, device):
    ...
    A = torch.randn(size=(n_samples, max_nodes, input_dim_A)).to(device)
    L = torch.randn(size=(n_samples, max_nodes, input_dim_L)).to(device)
    T = torch.randn(size=(n_samples, max_nodes, input_dim_T)).to(device)
    y = torch.zeros((n_samples, input_dim_y)).to(device)

    # Angle masks
    E = mirror(E)
    A_mask = get_triplet_mask(E.argmax(-1).long(), input_dim_A)
    L_mask = get_line_mask(E.argmax(-1).long(), input_dim_L)
    T_mask = get_twist_mask(E.argmax(-1).long(), input_dim_T)
...
def sample_reverse(con_diffusion, cat_diffusion, model, t, x_t):
    ...
    A_mask = get_triplet_mask(E.argmax(-1).long(), model.module.input_dims['A'])
    L_mask = get_line_mask(E.argmax(-1).long(), model.module.input_dims['L'])
    T_mask = get_twist_mask(E.argmax(-1).long(), model.module.input_dims['T'])
    A = con_diffusion.reverse_process(A, A_0, t, A_mask)
    L = con_diffusion.reverse_process(L, L_0, t, L_mask)
    T = con_diffusion.reverse_process(T, T_0, t, T_mask)
...
def sample_mols(x_0, dataset):
    ...
    A = (A * torch.pi * 0.5).clamp(min=-0.5*torch.pi, max=0.5*torch.pi) + 0.5 * torch.pi
    L = (L * torch.pi).clamp(min=-torch.pi, max=torch.pi)
    T = (T * torch.pi).clamp(min=-torch.pi, max=torch.pi)

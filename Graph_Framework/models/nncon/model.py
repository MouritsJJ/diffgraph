...
class NNCon(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_in_X = nn.Sequential(nn.Linear(self.input_dims['X'], hidden_mlp_dims['X']), nn.ReLU(),
                                      nn.Linear(hidden_mlp_dims['X'], hidden_dims['dx']), nn.ReLU())

        self.mlp_in_E = nn.Sequential(nn.Linear(self.input_dims['E'], hidden_mlp_dims['E']), nn.ReLU(),
                                      nn.Linear(hidden_mlp_dims['E'], hidden_dims['de']), nn.ReLU())

        self.mlp_in_y = nn.Sequential(nn.Linear(self.input_dims['y'], hidden_mlp_dims['y']), nn.ReLU(),
    ...
    def forward(self, X, E, y, node_mask):
        ...
        X = self.mlp_out_X(X) + x_out
        E = self.mlp_out_E(E) + e_out
        y = self.mlp_out_y(y) + y_out

        X = X * X_mask.float()
        E = E * E_mask.float() * diag_mask 
